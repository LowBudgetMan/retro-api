...
public record ApiTokenView(
    UUID id, String name, String tokenPrefix, Set<String> scopes, Instant createdAt
) {
    public static ApiTokenView from(ApiTokenEntity e) {
        Set<String> scopes = Arrays.stream(e.getScopes().split(","))
            .map(String::trim).filter(s -> !s.isEmpty()).collect(Collectors.toSet());
        return new ApiTokenView(e.getId(), e.getName(), e.getTokenPrefix(), scopes, e.getCreatedAt());
    }
}
